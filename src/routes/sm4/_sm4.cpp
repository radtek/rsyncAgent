...
//然后使用S盒进行非线性变换，再将变换结果转换为32比特的整数GET_ULONG_BE(bb,b,0)，最后对得到的32位整数bb进行线性变换： 
...
//轮函数F 每一轮加密中，输入为(x0, x1, x2, x3)，xi为32位比特，共计128比特。通过x0^sm4Lt(x1^x2^x3^rk)得到该轮加密的结果。
...
//变换T’ 先进行Sbox的非线性替换，然后进行线性变换，线性变换L改为了： rk = bb ^ (ROTL(bb, 13)) ^ (ROTL(bb, 23));
...
//设置密钥，对当前传入的主密钥进行32轮的迭代，每次迭代的轮密钥都被存放到ctx结构中的sk数组中
//类似于加密中的操作，首先通过宏将初始的密钥转换为4个32位bit整数，MK0，MK1，MK2，MK3，并为计算各轮密钥预先准备好初始值
static void sm4_setkey(unsigned long SK[32], unsigned char key[16])
{
	unsigned long MK[4]; //转换后的4个32bit秘钥
	unsigned long k[36];
	unsigned long i = 0;
	//线性变换 L 位移异或
...
//和该轮的密钥 sk[i]，产生出该轮的密文 ulbuf[i+4]，最后的密文存储在ulbuf[35]~ulbuf[32]中，转换为字符数组的形式放入output中。
